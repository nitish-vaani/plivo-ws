...
AGENT_CONNECTION_TIMEOUT = 5  # Reduced from 30s
MAX_CONNECTIONS = 500
...
class OptimizedMaqsamWebSocketHandler:
    """Optimized WebSocket handler with robust error handling and transfer support"""
    ...
    def __init__(self, websocket):
        ...
        logger.info("Created optimized WebSocket handler with transfer support")
    ...
    def _setup_room_events(self):
        """Setup LiveKit room event handlers including data channel support"""
        ...
        def on_data_received(data_packet):
            """Handle data messages from participants (including transfer requests)"""
            participant = data_packet.participant
            data = data_packet.data
            logger.info(f"Data received from {participant.identity}: {len(data)} bytes")
            
            # Handle transfer requests from agent
            asyncio.create_task(self._handle_data_message(data, participant))

    async def _handle_data_message(self, data, participant):
        """Process data messages from LiveKit participants"""
        try:
            # Decode the data message
            message_str = data.decode('utf-8')
            logger.info(f"Data message from {participant.identity}: {message_str}")
            
            # Handle simple transfer request
            if message_str == "TRANSFER_REQUEST":
                await self._handle_transfer_request(participant.identity, "agent_decision")
                return
            
            # Handle structured transfer data
            try:
                transfer_data = json.loads(message_str)
                if isinstance(transfer_data, dict) and transfer_data.get("action") == "transfer":
                    reason = transfer_data.get("reason", "agent_decision")
                    context = transfer_data.get("context", "")
                    await self._handle_transfer_request(participant.identity, reason, context)
                    return
            except json.JSONDecodeError:
                # Not JSON, might be other data message
                pass
            
            # Handle other data message types here if needed
            logger.debug(f"Unhandled data message: {message_str}")
            
        except Exception as e:
            logger.error(f"Error processing data message from {participant.identity}: {e}")

    async def _handle_transfer_request(self, requester_identity, reason="agent_decision", context=""):
        """Handle transfer request from agent"""
        if self.transfer_requested or self.transfer_in_progress:
            logger.warning(f"Transfer already requested/in progress, ignoring request from {requester_identity}")
            return
        
        # Validate that the requester is the agent
        if not self._is_agent_participant_by_identity(requester_identity):
            logger.warning(f"Transfer request from non-agent participant: {requester_identity}")
            return
        
        logger.info(f"Transfer request received from agent {requester_identity}, reason: {reason}")
        if context:
            logger.info(f"Transfer context: {context}")
        
        self.transfer_requested = True
        self.transfer_reason = reason
        
        try:
            # Send call.redirect to Maqsam
            success = await self._send_call_redirect(reason, context)
            
            if success:
                self.transfer_in_progress = True
                logger.info("Call transfer initiated successfully")
                
                # Optional: Send acknowledgment back to agent via data channel
                await self._send_transfer_acknowledgment(reason)
                
                # The call will be terminated by Maqsam after redirect
                # so we don't need to do cleanup here - it will happen naturally
            else:
                logger.error("Failed to send call.redirect to Maqsam")
                self.transfer_requested = False
    ...
    def _is_agent_participant_by_identity(self, identity):
        """Check if participant identity belongs to an agent"""
        identity_lower = identity.lower()
        agent_patterns = [
            "agent-", agent_name.lower(), "ac_", "agent", "assistant", "ai-"
        ]
        return any(pattern in identity_lower for pattern in agent_patterns)

    async def _send_call_redirect(self, reason="agent_decision", context=""):
        """Send call.redirect message to Maqsam"""
        try:
            if not self._is_websocket_open() or not self.session_ready:
                logger.error("Cannot send call.redirect: WebSocket not ready")
                return False
            ...
            logger.info(f"Sending call.redirect to Maqsam (reason: {reason})")
            if context:
                logger.info(f"Transfer context: {context}")
            ...
            logger.info("Call redirect sent successfully to Maqsam")
            return True
            
        except websockets.ConnectionClosed:
            logger.warning("WebSocket connection closed while sending redirect")
            self.call_active = False
            return False
        except Exception as e:
    ...
    async def _send_transfer_acknowledgment(self, reason):
        """Send transfer acknowledgment back to agent via data channel"""
        try:
            if not self.room or not self.connected_to_livekit:
                return
            ...
            await self.room.local_participant.publish_data(
                ack_data.encode('utf-8'),
                reliable=True
            )
            
            logger.debug("Transfer acknowledgment sent to agent")
            
        except Exception as e:
            logger.error(f"Error sending transfer acknowledgment: {e}")

    async def trigger_manual_transfer(self, reason="manual_test"):
        """Manual transfer trigger for testing purposes"""
        logger.info(f"Manual transfer triggered: {reason}")
        await self._handle_transfer_request("manual", reason, "Manual transfer for testing")
    ...
    async def cleanup(self):
        ...
        # Log transfer state if relevant
        if self.transfer_requested or self.transfer_in_progress:
            logger.info(f"Cleanup during transfer state - requested: {self.transfer_requested}, in_progress: {self.transfer_in_progress}")
        ...
        # Add transfer info to session summary
        if self.transfer_requested or self.transfer_in_progress:
            logger.info(f"  Transfer: requested={self.transfer_requested}, reason={self.transfer_reason}")
        else:
            logger.info(f"  Transfer: No transfer requests")
...
async def start_maqsam_websocket_server():
    ...
    try:
        async with websockets.serve(
            ...
            logger.info("Call transfer feature: ENABLED via data channels")
...
async def start_http_server():
    ...
    async def handle_health(request):
        return web.json_response({
            ...
            "version": "3.1-with-transfer-feature",
            "connections": {
...
async def main():
    ...
    logger.info("Starting Optimized Maqsam-LiveKit Bridge v3.1 with Transfer Feature...")
    logger.info("=" * 80)
    ...
    logger.info("Transfer Feature: ENABLED via LiveKit data channels")
    logger.info("=" * 80)
...
if __name__ == "__main__":
    try:
        ...
        if 'audio_processor_pool' in globals():
            audio_processor_pool.shutdown(wait=True)
        logger.info("Optimized Maqsam-LiveKit Bridge with Transfer Feature stopped")